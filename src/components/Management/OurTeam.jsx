...
const OurTeam = () => {
  return (
    <section className="bg-[#F2F2F2] pt-12 sm:pt-14 md:pt-16 pb-20 px-4 sm:px-6 lg:px-[90px]">
      <div className="max-w-[1200px] mx-auto">
        <div className="relative text-center mb-14 sm:mb-16 md:mb-20">
          <Heading className="relative z-10">Meet Our Team</Heading>
          <img
            className="absolute lg:left-[32%] sm:left-[29%] left-[6%]  top-[-22%]  md:w-[50px]"
            src={ellipseImg}
            alt="ellipse-img"
          />
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-x-8 gap-y-14 justify-items-center">
          {PROFILES_DATA.slice(0, 9).map((person, index) => (
            <div
              key={index}
        ...
        <div className="flex flex-col sm:flex-row justify-center gap-x-20 gap-y-12 mt-[117px]">
          {PROFILES_DATA.slice(9, 11).map((person, index) => (
            <div
              key={index}
              className="bg-white rounded-2xl shadow px-5 pt-20 pb-6 text-center w-full max-w-[264px] relative"
            >
